...
- Questions auto-advance after 2.5s (`setTimeout` at `app.js:1015`) — was 1.5s before fix
- **Testing tip:** Feedback (green/red button colors, "Correto!"/"Errado!" text) displays for ~2.5s. This is enough time for screenshots but still requires quick capture. Verify feedback mechanism by reading source code at `app.js:997-1015` if UI capture misses it.
...
- Widget renders in both quiz start view AND quiz results view (fixed: `#engagementWidget` div added to `renderQuizResult()` template at `app.js:1038`)
- After quiz completion, widget shows updated level/points reflecting quiz score bonus
